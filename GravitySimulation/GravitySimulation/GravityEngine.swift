...
struct BodyState {
    ...
    class GravityEngine {
        ...
        var maxInteractionsPerBody: Int? = nil
        ...
        private func step(showTrail: Bool) {
            ...
            for i in 0..<bodies.count {
                let a = bodies[i]
                let interactionBodies = getInteractionBodies(for: i)
                
                for j in interactionBodies {
                    let b = bodies[j]
            ...
            for i in 0..<bodies.count {
                let collidedWith = collisions.filter { $0.0 == i }.map {$0.1} +
                collisions.filter { $0.1 == i }.map {$0.0}
                snapshot.append(BodyState(position: bodies[i].position,
                                          velocity: bodies[i].velocity,
                                          collisionWith: collidedWith))
            }
            timeline.append(snapshot)
        }
        
        private func getInteractionBodies(for index: Int) -> [Int] {
            guard let maxN = maxInteractionsPerBody else {
                return (0..<bodies.count).filter { $0 != index }
                
            }
            
            let a = bodies[index]
            let distances = bodies.enumerated().compactMap { (j, other) -> (Int, Float)? in
                guard j != index else { return nil }
                let dist = length(other.position - a.position)
                return (j, dist)
            }
            
            let nearest = distances.sorted { $0.1 < $1.1 }.prefix(maxN)
            ...
            for i in 0..<bodies.count {
                ...
                for j in snapshot[i].collisionWith {
                    if i < j {
                        ...
                        showShockwave(at: (bodies[i].position + bodies[j].position)/2,
                                      size: size,
                                      in: bodies[i].node.parent)
